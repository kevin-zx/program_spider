...
func (t *torrent) String() string {
	err := mu.Insert("INSERT IGNORE INTO thunder_data (`link`,`name`,`length`,`file_count`) VALUE (?,?,?,?) ",
		fmt.Sprintf("magnet:?xt=urn:btih:%s", t.infohashHex),
			t.name,t.length,len(t.files))
	if err != nil {
		panic(err)
	}
	return fmt.Sprintf(
...
func main() {
	...
	maxFriends := flag.Int("f", 50, "max friends to make with per second")
	peers := flag.Int("e", 50, "max peers(TCP) to connenct to download torrent file")
